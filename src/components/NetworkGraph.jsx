...
export const NetworkGraphSpecial = (props) => {
  const networkRef = useRef(null);
  const { data, isLoading } = useNFTData();
  const navigate = useNavigate();
  const [isLoadingNetwork, setIsLoadingNetwork] = useState(true);
  ...
  useEffect(() => {
    ...
    const optionsZoom = {
      position: {x:0, y:0}, // ビューポートの中心位置
      scale: 1.3, // ズームレベル (1.0 で100%のズーム)
      offset: {x:0, y:0}, // 中心からのオフセット
    };


    network.once("initRedraw", () => {
      network.moveTo(optionsZoom);
    });
